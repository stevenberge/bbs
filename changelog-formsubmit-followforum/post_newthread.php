...
//////////////
if(isset($ispush)&&$ispush==1){
	if(empty($_G['forum']['pushsection'])) {
		showmessage("no pushsection");
	}
}
...
/////////////////////////////
/////submitcheck is used to decide whether the form is to be edited or to submitted
...
//		echo "will push !!!!!!!!!";
//	}
//	else if(!isset($ispush)){
//		echo "not set ispush!!!!!";
//	}else {
//		echo "ispush is not set to 1!!!!";
//	}
/////
	$pushstate=$ispush?1:0;
	if($ispush){
		if(!isset($_G['forum']['allowpush'])||$_G['forum']['allowpush']!=1){
			/////////////////////////////
			showmessage("you don't have the permission to push at this board");
			return;
		}
		$pushsection=$_G['forum']['pushsection'];
	}
	$newthread = array(
		'fid' => $_G['fid'],
		'posttableid' => 0,
		...
		'closed' => $closed ? 1 : 0,
		'pushstate'=>$pushstate  //pushstate=0:不推送，1推送失败，2推送成功
	);
	//////////插入新主题
	$tid = C::t('forum_thread')->insert($newthread, true);
	useractionlog($_G['uid'], 'tid');
	function push_thread_remote($feedSection,$feedTitle,$feedContent,$feedLink){
			$a = array('bbsCategory'=>@iconv("gbk","utf-8",$feedSection),'feedTitle'=>@iconv("gbk","utf-8",$feedTitle),'feedContent'=>@iconv("gbk","utf-8",$feedContent),'feedLink'=>$feedLink); //定义参数
		$a = @http_build_query($a); //把参数转换成URL数据
		$s = array('http' => array('method' => 'POST','header' => 'Content-type: application/x-www-form-urlencoded',
			'content' => $a));
		$s = stream_context_create($s);
		$u = 'http://219.223.222.231:8080/notification.do?action=send'; //此处必须为完整路径
		try{
			$r=file_get_contents($u,false,$s);
			if(empty($r))
				return true;
		}catch(Exception $e){ }
			return false;
	...
	if($ispush){
		////推送
		if(isset($pushsection))
			/////////////###
			if(push_thread_remote($pushsection,$subject,messagecutstr($message,100),"http://219.223.222.232/bbs/forum.php?mod=viewthread&tid=".$tid)){
				C::t('forum_thread')->update($tid,array('pushstate'=>2));//更新推送成功消息
		}
	}
