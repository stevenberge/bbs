...
//			$this->config['db'][1]['dbhost']="219.223.222.232";
//			$this->config['db'][1]['dbname']="android";
//			$this->config['db'][1]['dbpw']="123";
//			$this->config['db'][1]['dbcharset']="utf-8";
//			echo "discuz_application.php#_init_db:";
//			print_r($this->config['db']);
//			PDB::init($driver, $this->config['db']);
		}
	}
...
///////////////////////////加入新功能：用username和password登录
	private function _init_user() {
		if($this->init_user) {
			if($auth = getglobal('auth', 'cookie')) {
			  //echo "auth:".var_export($auth,true)."<br/>";
			  $auth = daddslashes(explode("\t", authcode($auth, 'DECODE')));
			}
			list($discuz_pw, $discuz_uid) = empty($auth) || count($auth) < 2 ? array('', '') : $auth;
			//////////
			if(isset($_GET['from_mobile'])&&isset($_GET['username'])&&isset($_GET['password'])){
				$username=$_GET['username'];
				$password=$_GET['password'];
				/////isuid=0 means username login 
				//codes below can be all found in function userlogin
				if(!function_exists('uc_user_login')) {
				  loaducenter();
				}
				$ucresult = uc_user_login(addslashes($username), $password, $isuid=0);// $checkques=0, $questionid='', $answer='', $ip);
				//var_dump($ucresult);echo "<br/>";
				$discuz_uid=$ucresult[0];
				//echo "user id:$discuz_uid<br/>";
