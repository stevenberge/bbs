...
class register_ctl {
  ...
  function register_remote($username,$userpwd,$email){
	$paras=array('bbs-password'=>'i_am_bbs','username'=>$username,'password'=>$userpwd,'email'=>$email);
	$r=@request_remote('user.do','register',$paras,'post');
	return $r;
  }
...
//	if(!isset($_POST)){
	  ...
	  ////////////////////////用户注册信息提交以后的处理过程
	  if($sendurl) {/////注册的邮箱审核功能，一般简单起见不开启这项功能
	  ...
	  $emailstatus = 0;
	  if($this->setting['sendregisterurl'] && !$sendurl) {//////跳过
	  ...
	  if($this->setting['regstatus'] == 2 && empty($invite) && !$invitestatus) {////跳过邀请注册功能
		showmessage('not_open_registration_invite');
	  }

	  if($bbrules && $bbrulehash != $_POST['agreebbrule']) {////跳过
		showmessage('register_rules_agree');
	  }

	  $activation = array();
	  if(isset($_GET['activationauth'])) {/////不能跳过
	  ...
	  if(!$activation) {
		...
		////showmessage("uid:".$uid.var_export($activation));
		////showmessage("helloworld");
	  }
	  //username已经被认可，用户名不会再有错
	  $_G['username'] = $username;

	  if(getuserbyuid($uid, 1)) { ////bbs_common_member已经存在相同uid(从uc_common_member插入得到的uid）
		if(!$activation) {
		  uc_user_delete($uid);
		}
	  ...
	  //////////////////////////////远程插入新的用户
	  if(!isset($_GET['no_sync'])||$_GET['no_sync']!='yes'){
		$r=self::register_remote($username,$opassword,$email);
		try{
		  $t=json_decode($r);
		  if(empty($t)||$t->state!='success'){
			uc_user_delete($uid); //////回退
			showmessage("remote register failed:$r");
		  }
		}
		catch(exception $e){
		  uc_user_delete($uid); //////回退
		  showmessage("remote register failed:exception parse:$r");
		}
	  }
