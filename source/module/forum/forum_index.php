...
$forumaccess=array();
	foreach($forum_access as $access) {
		$forumaccess[$access['fid']] = $access;
	}
	$forum_follow = array();
	$forum_follow=C::t('forum_rss')->fetch_all_by_uid($_G['uid']);
	$forumfollow=array();
	foreach($forum_follow as $follow){
		$forumfollow[$follow['fid']] = $follow;
	}
//	echo "forumaccess: ";print_r($forumaccess);echo "<hr>";
//	echo "forumfollow: ";print_r($forumfollow);echo "<hr>";

	$forum_fields = C::t('forum_forumfield')->fetch_all($fids);
//	echo"forums:";print_r($forums);echo"<hr>";
	foreach($forums as $forum) {
		if($forum_fields[$forum['fid']]['fid']) {
			$forum = array_merge($forum, $forum_fields[$forum['fid']]);
		}
////////////////////
		if($forumaccess[$forum['fid']]) {
			$forum = array_merge($forum, $forumaccess[$forum['fid']]);
		}
		if($forumfollow[$forum['fid']]){
			$forum['followed']=true;
		}
		
		//	print_r($forum);echo "<hr>";
	...
	unset($forum_follow, $forumfollow);
	unset($forumaccess);
...
//echo "forumlist:  ";print_r($forumlist);
?>
