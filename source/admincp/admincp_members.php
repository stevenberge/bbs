...
EOF;
	...
	if(!submitcheck('accesssubmit')) {
		...
		foreach(array('view', 'post', 'reply','push', 'getattach', 'getimage', 'postattach', 'postimage') as $perm) {
		...
		if($addfid && $_G['cache']['forums'][$addfid]) {
			...
			///////////获取用户在某版块推送权限
			$allowpushnew = !$_GET['allowpushnew'] ? 0 : ($_GET['allowpushnew'] > 0 ? 1 : -1);
			...
			if($allowviewnew == -1) {
				$allowpostnew = $allowreplynew = $allowpushnew = $allowgetattachnew = $allowgetimagenew = $allowpostattachnew = $allowpostimagenew = -1;
			} elseif($allowpostnew == 1 || $allowreplynew == 1 || $allowpushnew ==1 || $allowgetattachnew == 1 || $allowgetimagenew == 1 || $allowpostattachnew == 1 || $allowpostimagenew == 1) {
				$allowviewnew = 1;
			}

			if(!$allowviewnew && !$allowpostnew && !$allowreplynew && !$allowpushnew && !$allowgetattachnew && !$allowgetimagenew && !$allowpostattachnew && !$allowpostimagenew) {
				...
				$data = array('uid' => $_GET['uid'], 'fid' => $addfid, 'allowview' => $allowviewnew, 'allowpost' => $allowpostnew, 'allowreply' => $allowreplynew, 'allowpush' => $allowpushnew, 'allowgetattach' => $allowgetattachnew, 'allowgetimage' => $allowgetimagenew, 'allowpostattach' => $allowpostattachnew, 'allowpostimage' => $allowpostimagenew, 'adminuser' => $_G['uid'], 'dateline' => $_G['timestamp']);
