...
function showmessage($message, $url_forward = '', $values = array(), $extraparam = array(), $custom = 0) {
	///////////////
	if(isset($_REQUEST['from_mobile'])) {echo $message;exit(0);}
...
///////////////远程请求推送服务器，返回请求结果
  function request_remote($mod,$action,$parameters,$method='POST'){//$mod: follow.do action: followthread
//	$u = "http://219.223.222.231:8080/$mod?action=$action"; 
	$u = "http://219.223.222.231/push-api/index.php?mod=$mod&action=$action";
	//METHOD=POST
	if($method=='POST'||$method=='post'){
	  $a = @http_build_query($parameters); //把参数转换成URL数据
	...
	//METHOD=GET
	  ...
	  $s = array('http' => array('method' => 'GET','header' => 'Content-type: application/x-www-form-urlencoded'));
	  $s = stream_context_create($s);
...
function push_thread_remote($feedSection,$feedTitle,$feedContent,$feedLink){
	$a = array('feedSection'=>@iconv("gbk","utf-8",$feedSection),'feedTitle'=>@iconv("gbk","utf-8",$feedTitle),'feedContent'=>@iconv("gbk","utf-8",$feedContent),'feedLink'=>$feedLink); //瀹氫箟鍙傛暟
	$a = @http_build_query($a); //鎶婂弬鏁拌浆鎹㈡垚URL鏁版嵁
	$s = array('http' => array('method' => 'POST','header' => 'Content-type: application/x-www-form-urlencoded',
				'content' => $a));
	$s = stream_context_create($s);
	$u = 'http://219.223.222.231:8080/notification.do?action=send'; //姝ゅ蹇呴』涓哄畬鏁磋矾寰?
		try{
			$r=file_get_contents($u,false,$s);
			if(empty($r))
				return true;
