...
$modarray = array('ajax','announcement','attachment','forumdisplay',
	...
	'followforum','unfollowforum','unfollowallforum','repush','relatekw','relatethread','rss','topicadmin','trade','viewthread','tag','collection','guide'
	);
...
if($_G['member']['uid']!=0){
  ...
  else if($mod=='repush'&&isset($_G['tid'])){
	  $tid=$_G['tid'];
	  $fid=$_G['fid'];
	  $uid=$_G['uid'];
	  $thread=C::t('forum_post')->fetch_threadpost_by_tid_invisible($tid);
	  if($_G['uid']!=1&&$thread['authorid']!=$uid) showmessage("you(".$uid.") are not the admin(1), nor the author (".$thread['authorid'].")");
	  if($_G['forum']['allowpush']!=1&&$_G['uid']!=1) showmessage("you have no permission to push on this forum");  
	  $feedTitle=addslashes($thread['subject']);
	  $feedContent=$thread['message'];
	  $feedLink="http://219.223.222.232/bbs/forum.php?mod=viewthread&tid=".$_G['tid'];
	  $pushSection=C::t('forum_forum')->fetch_pushsection_by_fid($fid);
	  //showmessage($pushSection.":".$feedTitle.":".$feedContent.":".$feedLink); 
	  if(push_thread_remote($pushSection,$feedTitle,$feedContent,$feedLink)){
		C::t('forum_thread')->update($tid,array('pushstate'=>2));
	  }
	  $mod="viewthread";
  }
