...
0
|| checktplrefresh('./template/default/forum/forumdisplay_fastpost.htm', './template/default/common/seditor.htm', 1371452255, '5', './data/template/7_5_forum_forumdisplay_fastpost.tpl.php', './template/daren2_123', 'forum/forumdisplay_fastpost')
|| checktplrefresh('./template/default/forum/forumdisplay_fastpost.htm', './template/default/common/seccheck.htm', 1371452255, '5', './data/template/7_5_forum_forumdisplay_fastpost.tpl.php', './template/daren2_123', 'forum/forumdisplay_fastpost')
;?>
...
<!--////this will upload a new thread to server -->
...
<option value="0" selected="selected">Ñ¡ÔñÖ÷Ìâ·ÖÀà</option><?php if(is_array($_G['forum']['threadtypes']['types'])) foreach($_G['forum']['threadtypes']['types'] as $typeid => $name) { if(empty($_G['forum']['threadtypes']['moderators'][$typeid]) || $_G['forum']['ismoderator']) { ?>
...
<?php } ?>
<span>»¹¿ÉÊäÈë <strong id="checklen">80</strong> ¸ö×Ö·û</span>
</div>
...
<a href="forum.php?mod=post&amp;action=newthread&amp;fid=<?php echo $_G['fid'];?>" onclick="switchAdvanceMode(this.href);doane(event);">¸ß¼¶Ä£Ê½</a>
...
ÄúĞèÒªµÇÂ¼ºó²Å¿ÉÒÔ·¢Ìû <a href="member.php?mod=logging&amp;action=login" onclick="showWindow('login', this.href)" class="xi2">µÇÂ¼</a> | <a href="member.php?mod=<?php echo $_G['setting']['regname'];?>" class="xi2"><?php echo $_G['setting']['reglinkname'];?></a>
<?php } else { ?>
...
<?php } } else { ?>
ÄúÏÖÔÚÎŞÈ¨·¢Ìû¡£<a href="javascript:;" onclick="$('newspecial').onclick()" class="xi2">µã»÷²é¿´Ô­Òò</a>
<?php } ?>
...
<p class="ptm pnpost">
<!-- ////å‘è¡¨å¸–å­æŒ‰é’®ï¼Œå¦‚â€˜uidâ€™æœ‰æ•ˆï¼Œåˆ™å¯ä»¥ç›´æ¥å‘å¸–ï¼Œå¦åˆ™æ˜¾ç¤ºç™»å½•æ¡†-->
<button <?php if($fastpost) { ?>type="submit" <?php } elseif(!$_G['uid']) { ?>type="button" onclick="showWindow('login', 'member.php?mod=logging&action=login&guestmessage=yes')" <?php } ?>name="topicsubmit" id="fastpostsubmit" value="topicsubmit" tabindex="13" class="pn pnc"><strong>·¢±íÌû×Ó</strong></button>
...
<label><input type="checkbox" name="adddynamic" class="pc" value="1" <?php if($_G['forum']['allowfeed'] && !$_G['tid'] && empty($_G['forum']['viewperm'])) { ?>checked="checked"<?php } ?> />×ª²¥¸øÌıÖÚ</label>
<?php } ?>
</p>
</form>
</div>
