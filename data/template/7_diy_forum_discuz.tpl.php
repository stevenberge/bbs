...
0
|| checktplrefresh('./template/daren2_123/forum/discuz.htm', './template/daren2_123/common/header.htm', 1371452278, 'diy', './data/template/7_diy_forum_discuz.tpl.php', './template/daren2_123', 'forum/discuz')
|| checktplrefresh('./template/daren2_123/forum/discuz.htm', './template/daren2_123/common/pubsearchform.htm', 1371452278, 'diy', './data/template/7_diy_forum_discuz.tpl.php', './template/daren2_123', 'forum/discuz')
|| checktplrefresh('./template/daren2_123/forum/discuz.htm', './template/default/common/footer.htm', 1371452278, 'diy', './data/template/7_diy_forum_discuz.tpl.php', './template/daren2_123', 'forum/discuz')
|| checktplrefresh('./template/daren2_123/forum/discuz.htm', './template/default/common/header_common.htm', 1371452278, 'diy', './data/template/7_diy_forum_discuz.tpl.php', './template/daren2_123', 'forum/discuz')
;?>
...
<?php if($_G['setting']['taskon'] && !empty($_G['cookie']['taskdoing_'.$_G['uid']])) { ?><span class="pipe">|</span><a href="home.php?mod=task&amp;item=doing" id="task_ntc" class="new">进行中的任务</a><?php } if(($_G['group']['allowmanagearticle'] || $_G['group']['allowpostarticle'] || $_G['group']['allowdiy'] || getstatus($_G['member']['allowadmincp'], 4) || getstatus($_G['member']['allowadmincp'], 2) || getstatus($_G['member']['allowadmincp'], 3))) { ?>
...
<?php } ?>
<div id="wp" class="wp"  style="border: 3px solid #909498;padding:10px;background:#f0f0f0;margin-top:10px;"><?php echo adshow("subnavbanner/a_mu");?><div id="pt" class="bm cl">
...
<ul class="xl xl1 heatl"><?php if(is_array($_G['cache']['heats']['subject'])) foreach($_G['cache']['heats']['subject'] as $data) { ?><li><?php if($_G['adminid'] == 1) { ?><a class="d" href="forum.php?mod=misc&amp;action=removeindexheats&amp;tid=<?php echo $data['tid'];?>" onclick="return removeindexheats()">delete</a><?php } ?><a href="forum.php?mod=viewthread&amp;tid=<?php echo $data['tid'];?>" target="_blank" class="xi2"><?php echo $data['subject'];?></a></li>
<?php } ?>
</ul>
</div>
...
<?php } } ?>
<style>
.fl_row td { border-top: 1px solid #cdcdcd;}
                        .fl_tb td{ padding:10px;border-right:1px solid #cdcdcd;border-bottom:1px solid #cdcdcd;}
                        .tr2{ color:#393939;height:25px;}
                        .tr2 td,.tr2 th{ line-height:25px;border-bottom:1px solid #cdcdcd;padding:4px 26px 1px;border-right:1px solid #cdcdcd;}
                        .tr2 a{color:#000;margin:0 2px 0}
                        .tr2 .tac {text-align:left;padding-left:65px}
                        .tr2 .r {text-align:center;}
</style>
<div class="fl" style="border:none;background:#none;"><?php if(is_array($catlist)) foreach($catlist as $key => $cat) { ?><div class="bm bmw <?php if($cat['forumcolumns']) { ?> flg<?php } ?> cl" >
<div class="bm_h cl">
<span class="o">
...
<div id="category_<?php echo $cat['fid'];?>" class="bm_c" style="<?php echo $collapse['category_'.$cat['fid']];?> padding: 0 ;background: url(<?php echo IMGDIR;?>/wrap_bg.png) repeat-x 0 top;">
<table cellspacing="0" cellpadding="0" class="fl_tb"> 
                                        <?php if(!$cat['forumcolumns']) { ?>
<tr class="tr2">
<td width="*" class="tac" colspan="2">版块</td>
<td class="r">主题 / 帖数</td>
<td class="tal y-style f" style="border-right:none;">最后发表</td>
</tr>
<?php } ?><?php $i =1;?><?php if(is_array($cat['forums'])) foreach($cat['forums'] as $forumid) { ?><?php $forum=$forumlist[$forumid];?><?php $forumurl = !empty($forum['domain']) && !empty($_G['setting']['domain']['root']['forum']) ? 'http://'.$forum['domain'].'.'.$_G['setting']['domain']['root']['forum'] : 'forum.php?mod=forumdisplay&fid='.$forum['fid'];?><?php if($cat['forumcolumns']) { if($forum['orderid'] && ($forum['orderid'] % $cat['forumcolumns'] == 0)) { ?>
</tr>
<?php if($forum['orderid'] < $cat['forumscount']) { ?>
<tr class="fl_row">
<?php } } ?>
<td class="fl_g" width="<?php echo $cat['forumcolwidth'];?>">
<!--////<div class="fl_icn_g"<?php if(!empty($forum['extra']['iconwidth']) && !empty($forum['icon'])) { ?> style="width: {<?php echo $forum['extra']['iconwidth'];?>+20}px;"<?php } ?>>-->
<div class="fl_icn_g"<?php if(!empty($forum['extra']['iconwidth']) && !empty($forum['icon'])) { ?> style="width: 80px;"<?php } ?>>
<?php if($forum['icon']) { ?>
<?php echo $forum['icon'];?>
<?php } else { ?>
<!-- ///////push修改 a href="<?php echo $forumurl;?>"<?php if($forum['redirect']) { ?> target="_blank"<?php } ?>><img src="<?php echo IMGDIR;?>/forum<?php if($forum['folder']) { ?>_new<?php } ?>.gif" alt="<?php echo $forum['name'];?>" /></a> -->
<a href="<?php echo $forumurl;?>"<?php if($forum['redirect']) { ?> target="_blank"<?php } ?>><img src="<?php echo IMGDIR;?>/0<?php echo $i;?>.png" alt="<?php echo $forum['name'];?>" /></a><?php $i >= 9 ? $i = 1 : $i++;?><?php } ?>
</div>
...
<a href="forum.php?mod=redirect&amp;tid=<?php echo $forum['lastpost']['tid'];?>&amp;goto=lastpost#lastpost" class="xi2"><?php echo cutstr($forum['lastpost']['subject'], 30); ?></a> <cite><?php echo $forum['lastpost']['dateline'];?> <?php if($forum['lastpost']['author']) { ?><?php echo $forum['lastpost']['author'];?><?php } else { ?>匿名<?php } ?></cite>
<?php } else { ?>
...
<dd>
<!--////////////////-->
<?php if($forum['allowpush']||$_G['member']['adminid']) { ?>
<img src="static/image/push.png"/>
<?php } else { ?>
<img src="static/image/nopush.png"/>
<?php } if($forum['followed']) { ?>
<a href="forum.php?mod=unfollowforum&amp;fid=<?php echo $forum['fid'];?>"><font color='Green'>取消收听</font></a>
<?php } else { ?>
<a href="forum.php?mod=followforum&amp;fid=<?php echo $forum['fid'];?>"><font color='red'>收听</font></a>
<?php } ?>
</dd>
</dl>
</td>
...
<td onMouseOver="this.style.backgroundColor='#f5f5f5'" onMouseOut="this.style.backgroundColor='#ffffff'">
...
<?php echo $cat['endrows'];?>
             ...
             <div class="flin"></div><?php echo adshow("intercat/bm a_c/$cat[fid]");?><?php } ?>
</div>
